...
// Builder creates a new pubvibeISCream adapter.
func Builder(_ openrtb_ext.BidderName, adapterConfig config.Adapter, _ config.Server) (adapters.Bidder, error) {
	...
	if adapterConfig.ExtraAdapterInfo != "" {
		if err := json.Unmarshal([]byte(adapterConfig.ExtraAdapterInfo), &info); err != nil {
			return nil, fmt.Errorf("pubvibeISCream: failed to parse extra_info: %w", err)
		}
	}
...
// to the correct ISCream endpoint based on the publisher-supplied mediatype param.
// If mediatype is absent, banner imps default to Banner IA and video imps to Video IA.
func (a *adapter) MakeRequests(request *openrtb2.BidRequest, _ *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
	...
	for _, imp := range request.Imp {
		...
		default:
			// "banner_ia" or no mediatype — route by imp type
			if imp.Banner != nil {
...
func parseMediaType(imp openrtb2.Imp) (string, error) {
	...
	var pubvibeExt openrtb_ext.ExtImpPubvibeISCream
...
func buildRequest(request *openrtb2.BidRequest, imps []openrtb2.Imp, endpoint string) (*adapters.RequestData, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pubvibeISCream: marshal bidRequest: %w", err)
	}
...
// MakeBids unpacks the ISCream server response into Prebid bid objects.
func (a *adapter) MakeBids(request *openrtb2.BidRequest, _ *adapters.RequestData, responseData *adapters.ResponseData) (*adapters.BidderResponse, []error) {
	...
	if err := jsonutil.Unmarshal(responseData.Body, &response); err != nil {
		return nil, []error{&errortypes.BadServerResponse{
			Message: fmt.Sprintf("pubvibeISCream: failed to unmarshal response: %s", err),
		}}
	}
...
func getMediaTypeForBid(bid openrtb2.Bid) (openrtb_ext.BidType, error) {
	...
	default:
		return "", fmt.Errorf("pubvibeISCream: unsupported MType %d for bid %s", bid.MType, bid.ID)
	}
}
